...
pred_labels = []
with open('attributions_motif/label_predictions.txt', 'r') as output:
    for line in output:
        currentPlace = line[:-1]
...
for num in range(70, 100, 1):
    ...
    if (label_ground_truth == pred_labels[num]) and not (label_ground_truth == 'None') and not (label_ground_truth == 'ATG and TAA'):
...
#        img_attribution = NormalizeNumpy(img_attribution*img_attribution)
        ...
        img_attribution_shsm = np.array(Image.open('attributions_motif/all/' + str(num) + 'shsm.png'))
